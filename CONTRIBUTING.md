...
4. 创建 `develop` 到 `main` 的 Pull Request；Package CI 会使用稳定通道执行源码复验、Web 包真实
   安装验收和 Windows/Linux 桌面兼容包验收。
5. 全部门禁通过并补齐目标平台要求的桌面证据后，使用 merge commit 合并。

`npm run verify:release` 只是在本地提前复现当前平台发布路径的可选命令，不是发布的人工前置。

版本变更合入 `main` 后，Release Workflow 才能创建 `v<version>` 标签和以 Web `.tgz` 为默认资产的
GitHub Release。不要手工创建正式标签、覆盖已发布版本或上传本地构建产物。Electron 资产只能由
维护者对已有 Release 手动运行 `Publish desktop packages` Workflow 生成和附加。发布失败时在
`develop` 修复并提升补丁版本后重新发布。
